...
describe('Atlas app', () => {
  ...
  it('opens a lesson from the dock', () => {
    renderApp();
    fireEvent.click(screen.getAllByRole('button', { name: /What ClaimFarm is/i })[0]);
    expect(screen.getByRole('heading', { name: /What ClaimFarm is/i })).toBeTruthy();
  });
  ...
  it('moves to the next lesson with ArrowRight', () => {
    ...
    expect(screen.getByRole('heading', { name: /The problem and the product/i })).toBeTruthy();
  });
